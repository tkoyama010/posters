...
私は力学シミュレーションのソフトウェアエンジニアとして働いています。
...
"人間のためのVTK"\: 可視化ツールキット（VTK）の高レベルAPI
...
大規模／複雑なデータ形状に対応した、シンプルな3Dプロッティング機能
...
ユーザーは直感的な方法で簡単に共通のフィルターを適用することができます。
...
このモデルでは、x軸は左から右へ、y軸は下から上へ、z軸は画像から浮かび上がるようになっています。
...
このメッシュをx軸を中心に60度ごとに回転させると、プロットできます。
...
プロッターに軸アクターを追加し、軸の原点を回転点に設定します。
...
コードリスト \ref{ExtrudeRotateCode} スイープ回転メソッドを使って直線から "skirt" を
作成するポリゴンデータです (図\ref{ExtrudeRotateFigure}) 。
}
\end{block}
\end{textblock*}
\end{frame}
...
また、 \href{https://dev.pyvista.org/core/filters.html}{warp\_by\_vector()} 方式で抽出されたベクターファイルを持つことができます（コードリスト \ref{WarpVectorCode} 、図 \ref{WarpVectorFigure} ）。
...
より複雑な方法では、ビューアップベクター、クリッピングプレーン、カメラパースペクティブなど、コンピュータグラフィックスモデルを操作することができます（図 \ref{CameraFrustumFigure}）。
...
コードリスト\ref{CameraFrustumCode}カメラと視錐台を作成します。
